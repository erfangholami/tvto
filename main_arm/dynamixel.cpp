...
ReadWrite *serial = new ReadWrite();
#define ID					(2)
#define LENGTH				(3)
...
void dynamixel::move(int pos, int id, int speed = 150, int acc = 5) {
	dxl_write_word(id, 32, speed);
	//dxl_write_wordd(id, 73, acc);
	dxl_write_word(id, 30, pos);
}
int dynamixel::dxl_initialize(int devIndex, int baudnum)
{
	float baudrate;
	baudrate = 2000000.0f / (float)(baudnum + 1);

	if (serial->dxl_hal_open(devIndex, baudrate) == 0)
		return 0;
...
void dynamixel::dxl_terminate()
{
	serial->dxl_hal_close();
}
...
int dynamixel::dxl_get_rxpacket_length()
{
...
void dynamixel::dxl_write_wordd(int id, int address, int value)
{
	while (giBusUsing);
...
void dynamixel::syncWrite(int id1, int id2, int address, int value1, int value2)
{
	dxl_set_txpacket_id(BROADCAST_ID);
	dxl_set_txpacket_instruction(INST_SYNC_WRITE);
	dxl_set_txpacket_parameter(0, address);
	dxl_set_txpacket_parameter(1, 2);
	dxl_set_txpacket_parameter(2, id1);
	dxl_set_txpacket_parameter(3, dxl_get_lowbyte(value1));
	dxl_set_txpacket_parameter(4, dxl_get_highbyte(value1));
	dxl_set_txpacket_parameter(5, id2);
	dxl_set_txpacket_parameter(6, dxl_get_lowbyte(value2));
	dxl_set_txpacket_parameter(7, dxl_get_highbyte(value2));

	dxl_set_txpacket_length((2 + 1) * 2 + 4);
