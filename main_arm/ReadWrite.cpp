...
ReadWrite::ReadWrite(int comm, float baud)
{
	dxl_hal_open(comm, baud);
}
ReadWrite::ReadWrite() {

}
int ReadWrite::dxl_hal_open(int devIndex, float baudrate)
{
	// Opening device
...
void ReadWrite::dxl_hal_clear(void)
{
	// Clear communication buffer
	PurgeComm(ghSerial_Handle, PURGE_RXABORT | PURGE_RXCLEAR);
}

int ReadWrite::dxl_hal_tx(unsigned char *pPacket, int numPacket)
{
	// Transmiting date
...
int ReadWrite::dxl_hal_rx(unsigned char *pPacket, int numPacket)
{
	// Recieving date
	// *pPacket: data array pointer
	// numPacket: number of data array
	...
	//	SetCommMask(ghSerial_Handle, EV_RXCHAR);
	//	WaitCommEvent(ghSerial_Handle, &dwRead, NULL);
...
void ReadWrite::dxl_hal_set_timeout(int NumRcvByte)
{
	// Start stop watch
...
int ReadWrite::dxl_hal_timeout(void)
{
	// Check timeout
